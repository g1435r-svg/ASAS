...
When frozen as an EXE (PyInstaller), sibling executables ASAS_server.exe and
ASAS_cli.exe are used – no Python installation is required on the target machine.
When run as a plain script (development), sys.executable + the .py files are used.
"""
...
if getattr(sys, "frozen", False):
    ...
    # Sibling EXEs produced by the multi-exe PyInstaller build
    SERVER_EXE = os.path.join(BASE_DIR, "ASAS_server.exe")
    CLI_EXE    = os.path.join(BASE_DIR, "ASAS_cli.exe")
    _FROZEN    = True
else:
    BASE_DIR   = os.path.dirname(os.path.abspath(__file__))
    SERVER_EXE = os.path.join(BASE_DIR, "app.py")
    CLI_EXE    = os.path.join(BASE_DIR, "chat_cli.py")
    _FROZEN    = False
...
def _launch_cmd(target: str) -> list[str]:
    """Return the command list to launch a target (EXE or .py script)."""
    if _FROZEN:
        return [target]
...
class App(tk.Tk):
    ...
    def _check_model_on_start(self) -> None:
        ...
        if path:
            ...
            self.model_var.set(f"✅  מודל: {name}")
        else:
    ...
    def _launch_cli(self) -> None:
        if not self._assert_model():
            return
        env = {**os.environ, "MODEL_PATH": find_model()}
        try:
            subprocess.Popen(
                _launch_cmd(CLI_EXE),
                env=env,
